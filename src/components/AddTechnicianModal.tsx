...
import { X, Loader2, Wrench, Phone, Mail, CheckCircle } from 'lucide-react';
...
export default function AddTechnicianModal({ onClose, onCreated }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setDone(true);
      showToast(isAr ? 'تم إنشاء حساب الفني بنجاح' : 'Technician account created successfully', 'success');
      setTimeout(() => {
        onCreated();
        onClose();
      }, 1500);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                {isAr ? 'سيتم إنشاء حساب فني مع صلاحيات الميدان' : 'A field technician account will be created'}
              </p>
            </div>
          </div>
        ...
        {done ? (
          <div className="flex flex-col items-center justify-center py-16 gap-4">
            ...
            <p className="font-semibold text-slate-900 text-lg">
              {isAr ? 'تم إنشاء حساب الفني بنجاح!' : 'Technician account created!'}
            </p>
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={isAr ? 'سعيد محمد' : 'Said Mohammed'}
                  required
                  className="w-full border border-slate-200 rounded-xl ps-10 pe-4 py-2.5 text-sm focus:ring-2 focus:ring-green-500 outline-none"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="technician@bestco.jo"
                  required
                  className="w-full border border-slate-200 rounded-xl ps-10 pe-4 py-2.5 text-sm focus:ring-2 focus:ring-green-500 outline-none"
                  dir="ltr"
                />
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">
                {isAr ? 'سيستخدمها الفني لتسجيل الدخول للتطبيق' : 'The technician will use this to log into the app'}
              </p>
            </div>
            ...
            <div className="bg-green-50 border border-green-100 rounded-xl p-3 text-xs text-green-700 space-y-1">
              ...
              <p>✓ {isAr ? 'حساب تسجيل دخول للفني (auth)' : 'Technician login account (auth)'}</p>
              <p>✓ {isAr ? 'ملف تعريف بدور "فني" في النظام' : 'Profile with "technician" role in system'}</p>
              <p>✓ {isAr ? 'سيظهر في قائمة الفنيين عند إنشاء المواعيد' : 'Will appear in technician list when scheduling'}</p>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                type="submit"
                disabled={saving}
                className="flex-1 py-3 rounded-xl bg-green-600 hover:bg-green-700 disabled:opacity-60 text-white text-sm font-semibold transition flex items-center justify-center gap-2"
              >
