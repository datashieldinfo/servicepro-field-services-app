...
export default function TechnicianDashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [todayRes, historyRes, partsRes] = await Promise.all([
      supabase
        .from('appointments')
    ...
    const normalisedJobs = (todayRes.data ?? []).map(normalise) as Job[];
  ...
  async function openJobDetail(job: Job) {
    ...
    if (tasks && tasks.length > 0) {
      ...
      // Auto-create default water filtration checklist if none exist
  ...
  async function updateJobStatus(jobId: string, newStatus: JobStatus) {
    ...
    // Re-fetch tasks after status change — DB trigger may create job_tasks on in_progress
  ...
  async function handleSubmitJob() {
    ...
    // Complete appointment + save TDS to appointments row
    ...
    // Also save to job_readings for the customer service report
    ...
    // Auto service_request if TDS after > 150 ppm (BioFamily RO good output <50; alert at >150)
    ...
    // Schedule next appointment if date was selected
    if (nextVisit && selectedJob.customer_id) {
      ...
      // Update customers.next_appointment
    ...
    return (
      <button
        onClick={() => {
          ...
          // Auto-open detail modal when starting work
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">{t('technician.hey')}، {profile?.full_name?.split(' ')[0] ?? ''} — {t('technician.jobsScheduled', { count: jobs.length })}</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { val: completedToday, label: t('technician.completedTasks'), color: 'text-green-600' },
          { val: totalPartsUsed, label: t('technician.partsUsedCount'), color: 'text-blue-600' },
          { val: jobs.length, label: t('technician.totalVisits'), color: 'text-slate-700' },
        ].map(s => (
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {jobs.map(job => {
            ...
            return (
              <div key={job.id} className={`bg-white rounded-2xl shadow-sm border overflow-hidden transition-all ${isActive ? 'border-green-200 ring-1 ring-green-100' : isWaiting ? 'border-orange-200 ring-1 ring-orange-100' : 'border-slate-100 hover:shadow-md'}`}>
                {isActive && <div className="bg-green-600 px-5 py-1.5 flex items-center gap-2"><div className="w-2 h-2 bg-white rounded-full animate-pulse" /><span className="text-white text-xs font-semibold">{t('technician.currentlyActive')}</span></div>}
                {isWaiting && <div className="bg-orange-500 px-5 py-1.5 flex items-center gap-2"><Hourglass className="w-3 h-3 text-white" /><span className="text-white text-xs font-semibold">{t('technician.awaitingApproval')}</span></div>}
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-1">
                        <span className="flex items-center gap-1.5 text-sm font-bold text-slate-600"><Clock className="w-3.5 h-3.5" />{formatTime(job.scheduled_at)}</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
                    <div className="flex items-center gap-2 text-sm text-slate-600"><MapPin className="w-4 h-4 text-slate-400 flex-shrink-0" /><span className="truncate">{job.customers?.address || job.address || '-'}</span></div>
                    <div className="flex items-center gap-2 text-sm text-green-700 font-medium"><FileText className="w-4 h-4 text-green-500 flex-shrink-0" />{job.service_type}</div>
                  </div>
                  {job.notes && <p className="text-xs text-slate-500 bg-slate-50 rounded-lg px-3 py-2 border border-slate-100 mb-3">{job.notes}</p>}
                  <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                    <div className="flex items-center gap-2">
                      {job.customers?.phone && (
                        <a href={`tel:${job.customers.phone}`} className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-green-600 transition bg-slate-50 hover:bg-green-50 px-3 py-1.5 rounded-lg">
                          <Phone className="w-3.5 h-3.5" />{job.customers.phone}
                        </a>
                      )}
                      {(job.address || job.customers?.address) && (
                        <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(job.address || job.customers?.address || '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-700 transition bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg font-medium">
                          <Navigation className="w-3.5 h-3.5" />{t('technician.openMap')}
                        </a>
                      )}
                    </div>
                    {job.status !== 'completed' && job.status !== 'cancelled' && (
                      <button onClick={() => openJobDetail(job)} className="flex items-center gap-1 text-xs font-semibold text-green-600 hover:text-green-700 transition">
  ...
  const renderHistory = () => (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        {loading ? <div className="flex items-center justify-center py-10"><Loader2 className="w-6 h-6 text-green-600 animate-spin" /></div>
          : historyJobs.length === 0 ? <div className="py-10 text-center text-sm text-slate-400">{t('common.noData')}</div>
          : (
                ...
                <tbody className="divide-y divide-slate-50">
                  {historyJobs.map(job => (
                    <tr key={job.id} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="px-5 py-3"><span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-green-100 text-green-700"><CheckCircle className="w-3 h-3" />{t('status.completed')}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="bg-green-50 border border-green-100 rounded-2xl p-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[{ v: completedToday, l: t('technician.completedTasks') }, { v: totalPartsUsed, l: t('technician.partsUsedCount') }, { v: jobs.length, l: t('technician.totalVisits') }].map(s => (
            <div key={s.l} className="text-center"><p className="text-2xl font-bold text-green-700">{s.v}</p><p className="text-xs text-green-600 mt-1">{s.l}</p></div>
          ))}
        </div>
      </div>
    </div>
  ...
  const renderJobDetail = () => {
    ...
    return (
      ...
      <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
        <div className="bg-white w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl max-h-[92vh] overflow-y-auto">
          <div className="sticky top-0 bg-white border-b border-slate-100 px-5 py-4 flex items-center justify-between z-10">
            ...
            <button onClick={() => setSelectedJob(null)} className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center hover:bg-slate-200 transition"><X className="w-4 h-4 text-slate-600" /></button>
          </div>
          ...
          <div className="px-5 pt-4 pb-3 border-b border-slate-100 bg-slate-50">
            ...
            {isPhase2 && <p className="text-xs text-green-700 text-center mt-2 font-medium">{isAr ? 'وافق العميل — يمكنك البدء بالعمل' : 'Customer approved — proceed with work'}</p>}
          </div>
          ...
          <div className="p-5 space-y-6">
            ...
            {isAwaiting && (
              <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-orange-800 font-semibold text-sm"><Hourglass className="w-4 h-4" />{t('technician.awaitingApproval')}</div>
                  <button
                ...
                {selectedJob.approval_notes && <p className="text-sm text-orange-900 whitespace-pre-wrap bg-orange-100 rounded-xl px-4 py-3">{selectedJob.approval_notes}</p>}
                <p className="text-xs text-orange-600">{isAr ? 'في انتظار موافقة العميل. اضغط "تحقق من الحالة" بعد تأكيد العميل.' : 'Waiting for customer to approve. Tap "Check status" after the customer confirms.'}</p>
              </div>
            )}
            ...
            {(isPhase1 || isPhase2) && (
              <div>
                <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2"><ClipboardList className="w-4 h-4 text-green-600" />{t('technician.checklist')}</h3>
                {checklist.length === 0 ? <p className="text-sm text-slate-400 py-2">{t('common.noData')}</p> : (
                  <div className="space-y-2">
                    {checklist.map(item => (
                      <label key={item.id} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition ${item.completed ? 'bg-green-50 border-green-200' : 'bg-white border-slate-100 hover:border-green-200'}`}>
                        <input type="checkbox" checked={item.completed} onChange={() => toggleChecklistItem(selectedJob.id, item.id)} className="w-5 h-5 rounded-md border-slate-300 text-green-600 focus:ring-green-500" />
            ...
            {isPhase1 && (
              <div>
                <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2"><Package className="w-4 h-4 text-blue-600" />{t('technician.proposedParts')}</h3>
                <PartsSelector parts={proposedParts} inventory={inventoryParts} onAdd={val => setProposedPartsMap(prev => ({ ...prev, [selectedJob.id]: [...(prev[selectedJob.id] ?? []), val] }))} onRemove={idx => setProposedPartsMap(prev => ({ ...prev, [selectedJob.id]: prev[selectedJob.id].filter((_, i) => i !== idx) }))} color="blue" />
              </div>
            )}
            ...
            {isPhase1 && (
              <div>
                <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2"><FileText className="w-4 h-4 text-amber-600" />{t('technician.approvalNotes')}</h3>
                <textarea value={notes} onChange={e => setNotesMap(prev => ({ ...prev, [selectedJob.id]: e.target.value }))} placeholder={t('technician.approvalNotesPlaceholder')} rows={3} className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm resize-none focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>
            )}
            ...
            {isPhase2 && (
              <div>
                <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2"><Package className="w-4 h-4 text-green-600" />{t('technician.partsUsed')}</h3>
                <PartsSelector parts={actualParts} inventory={inventoryParts} onAdd={val => setPartsMap(prev => ({ ...prev, [selectedJob.id]: [...(prev[selectedJob.id] ?? []), val] }))} onRemove={idx => setPartsMap(prev => ({ ...prev, [selectedJob.id]: prev[selectedJob.id].filter((_, i) => i !== idx) }))} color="green" />
              </div>
            )}
            ...
            {isPhase2 && (
              <div>
                <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2"><CheckCircle className="w-4 h-4 text-teal-600" />{t('technician.tdsReadings')}</h3>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" min="0" value={tds.before} onChange={e => setTdsMap(prev => ({ ...prev, [selectedJob.id]: { ...(prev[selectedJob.id] ?? { before: '', after: '' }), before: e.target.value } }))} placeholder="450" className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-teal-500 outline-none" />
                  </div>
                  <div>
                    ...
                    <input type="number" min="0" value={tds.after} onChange={e => setTdsMap(prev => ({ ...prev, [selectedJob.id]: { ...(prev[selectedJob.id] ?? { before: '', after: '' }), after: e.target.value } }))} placeholder="0" className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-teal-500 outline-none" />
                  </div>
                </div>
            ...
            {isPhase2 && (
              <div>
                <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2"><FileText className="w-4 h-4 text-amber-600" />{t('technician.notesField')}</h3>
                <textarea value={notes} onChange={e => setNotesMap(prev => ({ ...prev, [selectedJob.id]: e.target.value }))} placeholder={t('technician.notesPlaceholder')} rows={2} className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm resize-none focus:ring-2 focus:ring-green-500 outline-none" />
              </div>
            )}
            ...
            {isPhase2 && (
              <div className="bg-slate-50 rounded-2xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold text-slate-800 text-sm flex items-center gap-2"><CalendarPlus className="w-4 h-4 text-blue-600" />{t('technician.followupToggle')}</h3>
                  <button
                ...
                {followup.enabled && (
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <input type="text" value={followup.reason} onChange={e => setFollowupMap(prev => ({ ...prev, [selectedJob.id]: { ...followup, reason: e.target.value } }))} placeholder={isAr ? 'سبب زيارة المتابعة...' : 'Reason for follow-up...'} className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                    </div>
                  </div>
                )}
              </div>
            ...
            {isPhase2 && (
              <div className="bg-blue-50 rounded-2xl p-4 space-y-2">
                <h3 className="font-semibold text-blue-800 text-sm flex items-center gap-2"><Calendar className="w-4 h-4 text-blue-600" />{t('technician.scheduleNextVisit')}</h3>
                <input
                  type="date"
                  min={minDate}
                  value={nextVisit}
            ...
            {isPhase2 && (
              <div>
                <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2"><Camera className="w-4 h-4 text-teal-600" />{t('technician.uploadPhotos')}</h3>
            ...
            {(isPhase1 || isPhase2) && (
              <div className="space-y-3">
                {/* A: Request Customer Approval — Phase 1 */}
                {isPhase1 && (
                ...
                {/* C: Send Feedback to Back Office */}
                <button
                ...
                {/* D: Cannot Complete — Return to Admin */}
                <button
        ...
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              ...
              <button onClick={() => setFeedbackModal(prev => ({ ...prev, open: false }))} className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center hover:bg-slate-200 transition"><X className="w-4 h-4 text-slate-600" /></button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <textarea value={feedbackModal.description} onChange={e => setFeedbackModal(prev => ({ ...prev, description: e.target.value }))} placeholder={isAr ? 'اكتب التفاصيل هنا...' : 'Write details here...'} rows={3} className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm resize-none focus:ring-2 focus:ring-amber-500 outline-none" />
              </div>
              <div>
              ...
              <button onClick={handleSendFeedback} disabled={submittingFeedback || !feedbackModal.description.trim()} className="w-full bg-amber-500 hover:bg-amber-600 disabled:opacity-60 text-white py-3.5 rounded-xl font-semibold text-sm transition flex items-center justify-center gap-2">
      ...
      {/* Cannot Complete — Return to Admin modal */}
      {returnModal.open && (
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <h3 className="font-bold text-slate-900">{isAr ? 'إعادة المهمة للمكتب' : 'Cannot Complete — Return to Admin'}</h3>
              <button onClick={() => setReturnModal(prev => ({ ...prev, open: false }))} className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center hover:bg-slate-200 transition"><X className="w-4 h-4 text-slate-600" /></button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <button onClick={handleReturnToAdmin} disabled={submittingReturn || !returnModal.reason} className="w-full bg-red-500 hover:bg-red-600 disabled:opacity-60 text-white py-3.5 rounded-xl font-semibold text-sm transition flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20 sm:pb-8">
      {isOffline && <div className="bg-amber-500 text-white px-4 py-2.5 text-center text-sm font-medium flex items-center justify-center gap-2"><WifiOff className="w-4 h-4" />{t('offline.banner')}</div>}
      <Navbar />
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {(activeTab === 'home' || activeTab === 'tasks') && renderMyDay()}
        {activeTab === 'history' && renderHistory()}
        {activeTab === 'settings' && <div className="text-center py-12"><Settings className="w-12 h-12 text-slate-300 mx-auto mb-3" /><p className="text-slate-500">{t('nav.settings')}</p></div>}
      </div>
      ...
      <div className="fixed bottom-0 inset-x-0 bg-white border-t border-slate-200 sm:hidden z-40">
        <div className="flex items-center justify-around py-2">
          ...
          ].map(tab => { const Icon = tab.icon; const active = activeTab === tab.id; return (
...
// Extracted parts selector to reduce repetition
function PartsSelector({ parts, inventory, onAdd, onRemove, color }: {
  ...
  return (
    <>
      <select value="" onChange={e => { if (e.target.value && !parts.includes(e.target.value)) onAdd(e.target.value); }} className={`w-full border border-slate-200 rounded-xl px-4 py-3 text-sm bg-white focus:ring-2 ${ring} outline-none`}>
        <option value="">— اختر قطعة —</option>
        {inventory.map(p => <option key={p.id} value={p.part_name}>{p.part_name} ({p.quantity} {p.unit}){p.selling_price != null ? ` — ${p.selling_price.toFixed(2)} JOD` : ''}</option>)}
      </select>
      {parts.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-3">
          {parts.map((part, idx) => (
            <span key={idx} className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border ${tagBg}`}>
              {part}<button onClick={() => onRemove(idx)} className="hover:text-red-500"><X className="w-3 h-3" /></button>
            </span>
          ))}
        </div>
      )}
